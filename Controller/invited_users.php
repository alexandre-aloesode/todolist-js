...
//Je commence par récupérer l'id de l'utilisateur qu'on souhaite inviter, puis ses invitations pour les mettre à jour à la fin
    $request_select_invited_data = "SELECT id, invitations FROM utilisateurs WHERE login = :login";

    $query_select_invited_data = $SQL->prepare($request_select_invited_data);

    $query_select_invited_data->execute(['login' => $_POST['add_invited_input']]);

    $result_select_invited_data = $query_select_invited_data->fetchAll();

//Et je rajoute à l'invité dans sa colonne invitations l'id de l'utilisateur
    $invitations_update = json_decode($result_select_invited_data[0][1]);
    ...
    $query_update_invitations->execute([
        ...
        'id' => $result_select_invited_data[0][0]
    ]);
...
//Puis je la mets à jour en ajoutant l'invité
    array_push($invited_update, $result_select_invited_data[0][0]);
